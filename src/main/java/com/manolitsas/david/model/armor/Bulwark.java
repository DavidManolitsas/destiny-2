...
@Slf4j
public class Bulwark extends BaseArmor {

  public Bulwark() {
    super(Archetype.BULWARK, Stat.HEALTH, Stat.propertyClass);
  }
  ...
  public ArmorStats getStats(ArmorStats stats, Armor armor, ArmorSlot slot) {
    ...
    switch (armor.getTier()) {
      case 1 -> {
        stats.setHealth(stats.getHealth() + T1_PRIMARY_STAT);
        stats.setPropertyClass(stats.getPropertyClass() + T1_SECONDARY_STAT);
      }
      case 2 -> {
        stats.setHealth(stats.getHealth() + T2_PRIMARY_STAT);
        stats.setPropertyClass(stats.getPropertyClass() + T2_SECONDARY_STAT);
      }
      case 3 -> {
        stats.setHealth(stats.getHealth() + T3_PRIMARY_STAT);
        stats.setPropertyClass(stats.getPropertyClass() + T3_SECONDARY_STAT);
      }
      case 4 -> {
        stats.setHealth(stats.getHealth() + T4_PRIMARY_STAT);
        stats.setPropertyClass(stats.getPropertyClass() + T4_SECONDARY_STAT);
      }
      case 5 -> {
        stats.setHealth(stats.getHealth() + T5_PRIMARY_STAT);
        stats.setPropertyClass(stats.getPropertyClass() + T5_SECONDARY_STAT);
      }
    }
